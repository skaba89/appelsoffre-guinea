...
const regionData = [
  { region: "Conakry", count: 58 },
  { region: "Kankan", count: 23 },
  { region: "Nzérékoré", count: 18 },
  { region: "Kindia", count: 15 },
  { region: "Boké", count: 12 },
  { region: "Labé", count: 9 },
  { region: "Faranah", count: 7 },
  { region: "Mamou", count: 5 },
];
          ...
          <AnimatedCardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Appels d'offres par région administrative</p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
              {regionData.map((region, i) => (
                <motion.div
                  ...
                  className="flex flex-col items-center gap-1.5 p-3 rounded-xl bg-muted/30 hover:bg-muted/60 transition-colors cursor-pointer group"
                >
                  <MapPin className="h-4 w-4 text-muted-foreground group-hover:text-primary transition-colors" />
                  <span className="text-lg font-bold text-foreground group-hover:text-primary transition-colors">{region.count}</span>
                  <span className="text-[10px] text-muted-foreground text-center leading-tight">{region.region}</span>
                </motion.div>
              ))}
            </div>
            {/* Mini bar chart for regions */}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={regionData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [`${value} AO`, "Appels d'offres"]}
                  />
                  <Bar dataKey="count" fill="#3b82f6" radius={[4, 4, 0, 0]} animationDuration={800} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </AnimatedCardContent>
