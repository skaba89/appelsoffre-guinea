...
class Category(Base):
    ...
    tenant_id: Mapped[str | None] = mapped_column(UUID(as_uuid=False), ForeignKey("tenants.id", ondelete="CASCADE"), nullable=True, index=True)
...
class TenderTagLink(Base):
    ...
    tender_id: Mapped[str] = mapped_column(UUID(as_uuid=False), ForeignKey("tenders.id", ondelete="CASCADE"), primary_key=True)
